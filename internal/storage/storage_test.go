...
func TestInitCreatesLayoutAndSchemaIdempotently(t *testing.T) {
	...
	if got := strings.Join(tables, ","); got != "schema_migrations" {
		t.Fatalf("tables = %q, want only schema_migrations", got)
	}
	...
	if count != 1 || version != 1 {
		t.Fatalf("migration rows = %d, version = %d; want 1, 1", count, version)
	}
}
...
func TestMigrateRejectsNewerSchemaWithoutMutation(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx,
		"INSERT INTO schema_migrations(version, applied_at) VALUES (2, 'test')"); err != nil {
