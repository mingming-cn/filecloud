...
首次 `v0.1.0` 托管 workflow 暴露 30 秒 CLI HTTP 超时早于 2 分钟服务端 Head 校验预算，以及 Windows 纳秒 deadline 测试依赖调度时序。提交 `b59d60e6a0dd03a983054894a902035e40fac23e` 修复后，Linux/ext4、macOS/APFS 和 Windows/固定 NTFS 再次完成完整统一门禁、required-pass 与各 113 条结构化证明；macOS 耗时 406 秒，Windows 在上述固定 NTFS 路径耗时约 883 秒。Windows 结果明确包含修复后的 `TestHeadValidationDeadlineReleasesWorkPool` pass 和真实 CLI 生命周期证明。
