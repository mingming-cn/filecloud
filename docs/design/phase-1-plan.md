...
7. **删除保护**
   - 候选提交删除已跟踪路径超过 100 个或当前路径数 10%（任一满足）时默认中止。
   - 用户必须用错误输出中的候选 Commit 前缀再次执行 `sync --confirm-delete <prefix>`；确认只对该候选有效。

1A 完成条件：两名用户互相不可见；同一资料库两个 Linux 客户端可在故障注入下收敛，所有确认内容可达，Head、Sync Base 和工作树断言通过。

## 1B：跨平台文件系统验证

1. 在 APFS 和 NTFS 上执行与 1A 相同的对象、锁、扫描、checkout 和崩溃测试。
2. spike 并冻结以下平台实现：no-follow/reparse point、文件身份、no-replace、父目录持久化、跨进程锁、占用文件 rename。
3. 文件系统不满足必要原子性时明确拒绝绑定，不做静默降级。
4. 第一阶段明确不支持 NFS、SMB、FAT/exFAT 和网络映射盘；后续必须独立验证后才能移出排除列表。

1B 完成条件：Linux、macOS、Windows 使用同一对象向量生成相同 ObjectId，并通过各自平台验收矩阵。
