...
Filecloud 采用 [MIT License](../LICENSE) 发布。每个正式归档均包含项目 `LICENSE`、`THIRD_PARTY_NOTICES.md`，以及该目标平台构建实际使用的第三方 Go 模块原始许可文件。
...
4. 每个平台二进制的初始化、用户创建、登录、资料库创建、绑定和同步冒烟测试。
5. 每个原生目标平台在二进制构建和冒烟后，根据该目标实际编译依赖生成并打包第三方许可材料。
